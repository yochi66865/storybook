...
export type TooltipPosition = 'top' | 'top-left' | 'top-right' | 'bottom' | 'bottom-left' | 'bottom-right' | 'left' | 'right' | 'auto';
...
export class AfUiTooltipDirective implements OnInit, OnDestroy {
    ...
    @Input('afuiTooltipAllowHideOnMouseOver') allowHideOnMouseOver: boolean = false;

    @Input('afuiTooltipPosition') set positionInput(position: TooltipPosition) {
        this.position = position;
        if (this.overlayRef) {  // Overlay exists, create a new one
            this.createOverlay();
        }
    }
    ...
    constructor(private overlay: Overlay, private elementRef: ElementRef, private overlayPositionBuilder: OverlayPositionBuilder) { }
    ...
    calculateTooltipPosition(tooltipPosition: TooltipPosition) {
        ...
        return { originX, originY, overlayX, overlayY, offsetX, offsetY, arrowPosition };
    }
    ...
    wClick(event: MouseEvent) {
        ...
        if (!this.tooltipComponentMouseOver &&
            this.tooltipVisible &&
    ...
    public showTooltip() {
        if (this.overlayRef && !this.overlayRef.hasAttached()) {
            this.tooltipRef = this.overlayRef.attach(new ComponentPortal(AfUiTooltipComponent));
            if (this.tooltipTrigger === 'hover') {
                this.tooltipRef.location.nativeElement.addEventListener('mouseleave', () => this.hideTooltip());
            }
            this.tooltipRef.instance.content = this.content;
            this.tooltipRef.instance.context = this.context;
            this.tooltipRef.instance.header = this.header;
            this.tooltipRef.instance.arrowPosition = this.showArrow ? this.arrowPosition : null;
    ...
    public hideTooltip() {
        if (this.tooltipRef && this.tooltipRef.instance.mouseIsOverTheTooltip && !this.allowHideOnMouseOver) return;
